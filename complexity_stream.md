...
> **현재 상태 — Phase 37:** RGB-D density pilot과 DINO의 물체 대응 진단을 완료함. 경계에 가까운 순수 patch·외형 차이가 큰 patch에서도 기존 DINO의 대응 정보를 확인했고, 같은 국소 count에서 접경 관계 수가 달라짐을 확인함. 다음은 **DINO를 고정한 채 혼합 patch·경계·물체 묶음 표현을 보완하는 단계**임. 최종 Complexity GT와 fusion의 효용은 후속 검증 대상임.
...
①–⑦은 아래 본문의 설명 순서임. 근접도와 정적 제거를 검사한 Phase 34–35는 별도로 시도한 관계 점수 후보이므로 옆 경로로 표시함. **⑦은 실제 완료한 공동 진단**이며, ⑤의 feature 집계 범위 비교와 ⑥의 다중 뷰 활용은 여전히 미실행 제안임.
...
flowchart TD
    ...
    N7["⑦ Phase 37 공동 진단 · 완료<br/>어려운 순수 patch 대응과 가시 접경을 별도 평가<br/>다음은 혼합 patch·경계·물체 묶음 표현 보완"]
...
**기존 DINO를 고정하고 혼합 patch·경계·물체 묶음 표현을 보완하는 것이 다음 구현 대상임.** Phase 37에서 경계 주변과 외형 변화가 있는 순수 patch의 대응을 확인했으므로, 같은 판별 과제를 다시 풀기 위해 encoder부터 바꾸지는 않음.

한 patch에 A·B가 함께 보일 때 양쪽 정보를 유지하는 soft grouping, RGB-D로 경계를 더 세밀하게 보완하는 방법을 검토함. RGB-D에서 관계를 직접 예측하는 방법도 가능하며, soft grouping을 유일한 해법이나 검증된 모델로 채택한 것은 아님. 비교에서는 같은 scene·정답·평가 영역을 유지하고 혼합 영역 coverage·접경 보존을 확인함. 물체 묶음을 출력하는 후보라면 그 분할·병합 오류도 평가해야 함. 현재 위치쌍 AUROC와 GT block 근사는 이 평가를 설계할 근거이며, 기존 density 모델의 실패 원인이나 새 grouping 모델의 성능을 측정한 결과와 구분함.

⑤의 **같은 위치쌍·정답을 고정한 48/96/160px feature 집계 범위 비교**는 미실행 상태로 보존함. 주변 문맥 부족이 확인되면 세 유형의 validation AUROC 평균과 두 종류 판정 오류를 기준으로 비교할 수 있음. 기존 count 결과와 Phase 37의 GT block 오차는 평가할 정답·유효 위치가 달라 최적 window의 근거로 쓰지 않음.

⑥의 다중 뷰 teacher와 추가 모델 B/C도 아직 실행하지 않았음. 다른 view가 구체적 실패를 보완하는 경우에만 그 정보를 단일-view 모델의 학습에 전달하는 비교를 검토하며, 추론은 한 시점의 RGB-D를 유지함. Count와 가시 관계의 추가 정보를 감독 후보로 분리해 두고, 최종 Complexity GT·fusion·탐색 효용은 이후 검증함. 근접도·제거 횟수·뷰 합의를 임의로 새 정답으로 대체하지 않음.
