...
> **현재 상태 — Phase 38:** Density pilot·DINO 대응 진단에 이어 전체 RGB-D 영상의 접경 예측을 평가함. 작은 decoder에 depth를 추가한 exact-F1은 RGB `0.313310` 대비 `0.314580`으로, **3 seeds 중 1개만 개선되어 사전 채택 기준을 통과하지 못함**. 별도의 GT 관계 진단에서는 면적·개수 이상의 정적 제거 효용 정보를 확인함. 다음은 **기하적 경계 위치와 물체 소속·전경 판단을 함께 보존하는 방법**을 검증하는 단계이며, 새 architecture·최종 Complexity GT·fusion은 아직 선택·검증하지 않음.
...
관계를 표현하려는 방향에는 근거가 생겼고, 남은 핵심은 **실제 RGB-D에서 그 관계를 보존하는 것**임. Depth의 경계 위치를 유지하면서 물체–배경, 같은 물체의 내부 단차, 서로 다른 물체의 접경을 구분하는 방법을 비교함. Depth 차이가 작은 접경도 계속 평가함.
...
**기하 경계의 위치 정보와 물체 소속·전경 판단을 결합하고, 예측 관계가 제거 효과의 정보를 보존하는지 확인하는 것임.** Phase 38에서 GT 관계의 유용성은 확인했지만 작은 RGB-D concat decoder는 RGB 대비 일관된 개선을 보이지 못했음.
