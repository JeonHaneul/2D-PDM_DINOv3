...
①–⑥은 아래 본문의 설명 순서임. 근접도와 정적 제거를 검사한 Phase 34–35는 별도로 시도한 관계 점수 후보이므로 옆 경로로 표시함. ⑤의 feature 집계 범위 비교와 ⑥의 다중 뷰 활용은 물체 구분 실패를 확인한 뒤 검토할 미실행 제안임.
...
먼저 실제 더미의 경계·분리된 조각에서 같은 물체를 나누거나 다른 물체를 합치는 실패를 확인함. 주변 문맥의 범위가 부족한지, 다른 관측이 필요한지에 따라 ⑤·⑥의 미실행 비교안을 적용함.
...
⑥은 다른 view가 해당 실패를 보완하는지 확인한 뒤, 그 정보를 단일-view 모델의 학습에 활용할 수 있는지 검토하는 안임. 다중 camera를 추론의 필수 조건으로 바꾸지 않고 추가 감독의 이득을 확인하려는 방향임. 최종 Complexity와 fusion의 효용은 그 다음 평가 범위이며, 근접도·제거 횟수·뷰 합의를 새 GT로 바로 대체하지 않음.
