...
def _strip_nested_rigid_bodies(stage, prim_path: str) -> None:
    """prim_path 자신은 그대로 두고, 그 하위(자손) prim들에서만 RigidBodyAPI를 제거.

    일부 USD 에셋(예: World1.usd 계열 - 내부에 SubUSDs/model.usd로 물리 바디가 이미
    적용된 채 export됨)은 최상위 prim 밑의 child prim에도 RigidBodyAPI가 같이 구워져
    있다. 여기에 SingleRigidPrim으로 최상위 prim에 또 RigidBodyAPI를 적용하면 같은
    계층 안에 rigid body가 중첩되어 PhysX가 "missing xformstack reset" 에러를 내고
    시뮬레이션 결과를 예측할 수 없게 된다. object 하나당 물리 바디는 최상위 prim에만
    있어야 하므로, 로드 직후 자손들의 RigidBodyAPI만 제거해 항상 안전하게 만든다."""
...
class ObjectSpawner:
    ...
    def _create_prims(self) -> None:
        ...
        for i, (name, asset_path) in enumerate(asset_items):
            ...
            _strip_nested_rigid_bodies(stage, prim_path)
